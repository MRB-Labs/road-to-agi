...
- Run `git diff --check`.
- Push to `preview` or open a pull request when the change is not trivial.
- Let CI pass.
- Merge/push to `main`.
...
- Run the external citation checker during periodic link-rot reviews, not as a
  hard deploy blocker:
...
- Bump `content/REVISION` only for a named public revision.
