...
const COMETA={
 ...
 '3|KLA':['United States','>50% of process control (2024, The Information Network)'],
 '3|ASE Technology':['Taiwan','≈45% of the top ten OSATs by revenue (2024, TrendForce)'],
 ...
 '4|Arista Networks':['United States','Leads the data centre Ethernet switch market (3Q 2025, Dell’Oro)'],
 '5|Alphabet':['United States','≈15% of cloud infrastructure (Q2 2026, Synergy)'],
 '5|Microsoft':['United States','≈20% of cloud infrastructure (Q2 2026, Synergy)'],
 '5|Amazon':['United States','≈28% of cloud infrastructure (Q2 2026, Synergy)'],
 ...
 '5|Equinix':['United States','Clear worldwide leader in retail colocation'],
...
2:{lead:'One long chain from rock to qualified input, plus a grouping by where the material ends up. The audit’s correction matters here: extraction can be a chokepoint, and qualification is a gate that recurs rather than a final stage. Several of these materials are recovered as by-products, so supply cannot respond to their own price.',
 branches:[
  ...
  {t:'Where the material ends up',w:'The same chain, grouped by functional destination rather than by processing step.',
   ...
   {t:'Optical and communications materials',w:'Glass preforms, drawn fibre and the compound semiconductor substrates behind lasers and detectors.',n:['Corning','Fujikura','Sumitomo Electric','Shin-Etsu'],note:'Feeds layer 9'},
...
3:{lead:'Three branches converge on a finished device. Design and manufacturing inputs are parallel — EDA houses do not buy from equipment makers — and they meet at the fab. The audit’s correction stands: which step rations accelerators is a dated, conditional question, not a fixed answer.',
 branches:[
  ...
  {t:'What comes out',w:'The device categories this layer delivers into the rest of the stack.',
   ...
   {t:'Optical DSPs, drivers and detectors',w:'The silicon and compound-semiconductor devices behind every optical link.',n:['Broadcom','Marvell','Coherent','Lumentum','ams OSRAM'],note:'Into layer 9'},
...
5:{lead:'A physical enclosure with a lifecycle, and a set of commercial models that are routinely confused with it. Operator and tenant are not construction stages. Layer 5 contains the on-site part of layer 1, central layer 4, and layers 6, 7 and 8 running on it — containment is not ownership, and different companies may own the land, the building, the plant, the hardware and the workload.',
 branches:[
  ...
  {t:'What the enclosure contains',w:'Physical containment, not ownership. These layers run inside this building.',
   ...
   {t:'Layer 6 data systems',w:'Databases, object storage, data lakes and the ingestion and governance systems around them.',n:['Snowflake','Databricks','VAST Data','Pure Storage','NetApp','AWS','Google Cloud','Azure'],note:'Runs on layer 4, inside layer 5'},
   {t:'Layer 7 training and cloud inference',w:'Pre-training, post-training, evaluation and model serving.',n:['OpenAI','Anthropic','Google DeepMind','Meta','xAI','Azure','AWS','Google Cloud'],note:'Weights are portable; the compute is not'},
   {t:'Layer 8 cloud agents and applications',w:'Agent runtimes, orchestration, enterprise applications and observability.',n:['Microsoft','ServiceNow','Salesforce','Datadog','Palantir'],note:'Some agents stay here; others deploy to the edge'}]}]},
6:{lead:'The second new layer. Data was a single card inside the model layer, which understated it: data supports evaluation, retrieval, monitoring, personalisation, simulation and safety investigation, not only training. The audit is firm on the distinction this layer exists to enforce — possessing data is not owning it, and owning it is not having the right to train on it.',
 branches:[
...
7:{lead:'Compute and data are parallel inputs to model development — neither is downstream of the other, which the old chain implied. Training and inference are also different markets with different workload shapes, hardware economics and customers. Weights are portable artifacts, so this layer runs in the data centre and, optimised, inside the machine at the edge.',
 branches:[
  {t:'Research to deployed capability',w:'The development chain. Compute and data enter it in parallel at the start.',
   ...
   {t:'Data and mixture preparation',w:'The curated corpora arriving from layer 6, with the mixture design that decides what the model actually learns.',n:['Scale AI','Surge','Mercor','Licensed corpora','Synthetic generation'],note:'A parallel input, not downstream of compute'},
  ...
  {t:'Three deployment modes',w:'The same weights, three physical placements with different constraints.',
   ...
   {t:'Cloud inference',w:'Served from layer 5 across layer 9. Latency-sensitive, and priced per token in a market that has repriced repeatedly.',n:['Azure','AWS','Google Cloud','Together','Fireworks','Baseten'],note:'Latency-bound and continuously repriced'},
...
8:{lead:'Model outputs become controlled work here. The correction the audit insists on: security, identity, evaluation and observability are not final stages purchased after the application — they run across the whole layer as a control plane. And the tool and integration layer, which was missing entirely, is what makes an agent useful at all.',
 branches:[
  {t:'From model access to operated work',w:'The execution chain. Every stage in it is subject to the control plane below.',
   ...
   {t:'Context, retrieval and memory',w:'Assembling what the model sees: retrieved documents, short-term state and long-term memory, subject to the customer’s permissions.',n:['Elastic','Pinecone','MongoDB','Snowflake','Databricks'],note:'Reaches into layer 6, under the customer’s access rules'},
...
9:{lead:'A new layer, and one the earlier map did not have. Connectivity was partly hidden inside compute silicon and partly missing altogether. It spans the rack, the building, the carrier network and the link to a machine in the field — and the audit’s warning applies throughout: link speed is not application performance, and an autonomous machine cannot assume the cloud is reachable.',
 branches:[
  ...
  {t:'What actually flows',w:'Three traffic classes with different directions, volumes and failure consequences.',
   stages:[
   {t:'Sensor and experience data, edge to centre',w:'Selected telemetry and experience travelling from layer 10 to layer 6. Selected, because uploading raw sensor streams is neither affordable nor lawful in most settings.',n:['On-robot selection','Starlink','AT&T','Verizon','Cloudflare'],note:'Selection happens on the machine, not in the cloud'},
   {t:'Models, policies and software updates, centre to edge',w:'Validated weights and software returning from layers 7 and 8 to the machine. Signed, staged and reversible, or it is a fleet-wide outage.',n:['Cloudflare','Akamai','Qualcomm','NVIDIA Jetson'],note:'An update path is also an attack surface'},
...
10:{lead:'Six component branches converge at machine integration, then a lifecycle chain runs from integration to refurbishment. Inside the machine, sensors feed edge compute, which feeds local models, which feed control software, which drives actuators — and the loop closes back through layers 6, 7 and 8. Two claims from the old chain are gone: not every actuator needs a rare-earth magnet, and no universal BOM percentage applies across robot types.',
 branches:[
  {t:'Component branches',w:'Six parallel subsystem inputs. They converge at integration; none is upstream of another.',
   ...
   {t:'Edge compute, memory and communications',w:'The SoC or accelerator, memory, local storage, real-time I/O and the radios. Layer 4 and layer 9, at machine scale.',n:['NVIDIA Jetson','Qualcomm','Ambarella','Hailo','Tesla FSD silicon','STMicroelectronics'],note:'Layer 4 inside layer 10'},
  ...
  {t:'Integration and lifecycle',w:'From components to a fleet in service, and back again.',
   ...
   {t:'Model and software integration',w:'Perception, world models, policies and the agent logic from layers 7 and 8, running locally.',n:['NVIDIA','Figure','Tesla','Boston Dynamics','Google DeepMind'],note:'Layers 7 and 8 inside layer 10'},
  ...
  {t:'Inside the machine',w:'The internal containment the audit requires, and the loop it closes.',
   ...
   {t:'Local models — layer 7',w:'Perception, world model and policy running locally, because a safety-critical loop cannot wait for the cloud.',n:['NVIDIA','Google DeepMind','Tesla','Figure'],note:'Layer 7, optimised and local'},
   {t:'Agent and control — layer 8',w:'Planning, tool use, safety supervision and control coordination on the machine itself.',n:['Boston Dynamics','Tesla','Figure','Apptronik'],note:'Layer 8, with a defined safe state'},
...
1:{primary:{n:'Reliable delivered electricity productivity',
 ...
 worked:{t:'Delivered power project, lifecycle view',as_of:'Illustrative 2026',
   s:['Discounted lifecycle cost $1.0bn','Discounted SLA-compliant delivered energy 5.0bn kWh'],
   calc:[['Put useful output on top','5.0bn delivered kWh'],
     ['Put all lifecycle cost underneath','$1.0bn'],
     ['Divide output by cost','5.0bn kWh ÷ $1.0bn = 5.0 kWh/$'],
     ['Invert it for the familiar cost number','$1.0bn ÷ 5.0bn kWh = $0.20/kWh']],
...
2:{primary:{n:'Qualified material productivity',
 ...
 worked:{t:'Refining feed into qualified material',as_of:'Illustrative 2026',
   s:['1,000 t of feed','80% recovered','95% passes qualification','$3.8m total cost'],
   calc:[['Recover the target material','1,000 t × 80% = 800 t recovered'],
     ['Apply downstream qualification','800 t × 95% = 760 t qualified'],
     ['Convert tonnes to kilograms','760 t = 760,000 kg'],
     ['Divide qualified mass by cost','760,000 kg ÷ $3,800,000 = 0.20 kg/$'],
     ['Invert for unit cost','$3,800,000 ÷ 760,000 kg = $5/kg']],
...
3:{primary:{n:'Known-good packaged devices per dollar',
 ...
 worked:{t:'Turning gross dies into known-good packaged devices',as_of:'Illustrative 2026',
   s:['600 gross dies','80% fab yield','98% sort','95% package','99% final test','$20,000 wafer plus allocated test and packaging'],
   calc:[['Start with gross dies','600'],
     ['After fab yield','600 × 80% = 480'],
     ['After electrical sort','480 × 98% = 470.4'],
     ['After package yield','470.4 × 95% = 446.9'],
     ['After final test','446.9 × 99% = 442.4 good devices'],
     ['Divide cost by good devices','$20,000 ÷ 442.4 = $45.21 per good device']],
...
4:{primary:{n:'Useful work per dollar, and per joule',
 ...
 worked:{t:'Inference throughput under a latency SLA',as_of:'Illustrative 2026',
   s:['10,000 output tokens/s','8,000 W','92% meet the latency SLA'],
   calc:[['Gross energy productivity','10,000 tokens/s ÷ 8,000 W = 1.25 tokens/J'],
     ['Apply the acceptance gate','1.25 × 92% = 1.15 accepted tokens/J'],
     ['Equivalent view per second','10,000 × 92% = 9,200 accepted tokens/s']],
...
5:{primary:{n:'SLA-compliant IT work per facility dollar',
 ...
 worked:{t:'Adding facility overhead to IT energy',as_of:'Illustrative 2026',
   s:['Layer 4 uses 1.0 J per token','PUE = 1.20'],
   calc:[['Start with IT energy','1.0 J/token'],
     ['Apply facility overhead','1.0 J/token × 1.20 = 1.20 J/token'],
     ['Read the overhead','0.20 J/token is power conversion, cooling and facility loss']],
...
6:{primary:{n:'SLA-compliant payload goodput per dollar',
 ...
 worked:{t:'Nominal link speed versus useful payload',as_of:'Illustrative 2026',
   s:['85 Gbit/s of payload','1,000 W','nominal capacity 100 Gbit/s'],
   calc:[['Energy productivity','85 Gbit/s ÷ 1,000 W = 0.085 Gbit/J'],
     ['Convert units','0.085 Gbit/J = 85 Mbit/J'],
     ['Payload utilisation','85 Gbit/s ÷ 100 Gbit/s = 85%'],
     ['What is still missing','Latency, packet loss and jitter must be reported beside this']],
...
7:{primary:{n:'Successful governed retrievals per dollar',
 ...
 worked:{t:'Why governance is a gate, not a footnote',as_of:'Illustrative 2026',
   s:['Retrieval and quality scores multiply, they do not average','A corpus with perfect recall and no usage rights scores zero'],
   calc:[['Example retrievals','1,000,000 attempted retrievals'],
     ['Apply retrieval score','1,000,000 × 90% = 900,000 useful matches'],
     ['Apply quality score','900,000 × 85% = 765,000 quality-adjusted matches'],
     ['Apply rights/governance score','765,000 × 0% = 0 accepted retrievals'],
     ['Reason','If it cannot legally or safely be used, it is not useful output']],
...
8:{primary:{n:'Successful quality-constrained tasks per lifecycle dollar',
 ...
 worked:{t:'Model serving cost after quality acceptance',as_of:'Illustrative 2026',
   s:['1,000,000 requests','80% acceptance','$20,000 lifecycle cost allocated to the period'],
   calc:[['Accepted task count','1,000,000 × 80% = 800,000 accepted tasks'],
     ['Productivity','800,000 ÷ $20,000 = 40 accepted tasks/$'],
     ['Invert for unit cost','$20,000 ÷ 800,000 = $0.025 per accepted task']],
...
9:{primary:{n:'Completed end-to-end workflows per dollar',
 ...
 worked:{t:'Workflow automation after completion and acceptance',as_of:'Illustrative 2026',
   s:['100,000 initiated workflows','88% completion','97% acceptance','$60,000 full cost'],
   calc:[['Completed workflows','100,000 × 88% = 88,000 completed'],
     ['Accepted completed workflows','88,000 × 97% = 85,360 accepted'],
     ['Productivity','85,360 ÷ $60,000 = 1.42 accepted workflows/$'],
     ['Invert for unit cost','$60,000 ÷ 85,360 = $0.703 per accepted workflow'],
     ['Reliability intuition','20 dependent steps at 99% each: 0.99^20 = 81.8% end-to-end']],
...
10:{primary:{n:'Accepted physical work per dollar, and per hour',
 ...
 worked:{t:'Physical task productivity from an end-to-end trial',as_of:'Illustrative 2026',
   s:['Perception, decision, control, actuation and acceptance rates each below 1','Estimate from end-to-end trials, not by multiplying component rates'],
   calc:[['Trial setup','1,000 pick attempts during 50 operating hours'],
     ['Accepted physical output','920 picks pass the end-market acceptance rule'],
     ['Good-task rate','920 ÷ 50 h = 18.4 accepted picks/hour'],
     ['Allocated trial-period cost','$4,600'],
     ['Unit cost','$4,600 ÷ 920 = $5.00 per accepted pick'],
     ['Why not multiply sub-systems','Component rates diagnose the failure; the end-to-end trial is the measurement']],
...
['6 Data and knowledge infrastructure','None direct — inherits storage media, compute and power','The binding constraint here is legal rather than material: the right to use the data, and lineage that cannot be added afterwards','None direct'],
['7 AI models and inference','None','No physical barrier to entry — which is the finding, not a gap','None'],
['8 Agentic software and applications','None','The two layers with no material chokepoint are the two with the weakest moats','None'],
['9 Connectivity and communications','High-purity silica, germanium dopant, indium phosphide and gallium arsenide, copper','Fibre draw capacity, by-product germanium — and, hardest of all, rights of way, which are not a material at all','Moderate'],
...
9:{lead:'Almost the whole layer runs on one material — high-purity silica drawn into fibre — modified by dopants present in fractions of a percent. The elements that matter are the ones that change the refractive index and the ones that make light on the other end of the glass.',
 els:[
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   the facility that physically cause it, never twice — layers 6, 7 and 8 have
